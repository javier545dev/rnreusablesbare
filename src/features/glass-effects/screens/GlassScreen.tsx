...
export default function ExamplesScreen() {
  ...
  return (
    <ImageBackground source={{ uri: DEMO_IMAGES[1] }} className="flex-1" resizeMode="cover">
      <Animated.ScrollView
        contentContainerClassName={'px-6 pb-24'}
        onScroll={scrollHandler}
        scrollEventThrottle={16}
      >
        <Text className="text-3xl font-bold text-white text-center mt-10 mb-5">Blur Examples</Text>

        <Text className="text-xl font-bold text-white mt-5 mb-4">Blur Switch</Text>
        <BlurView blurType="dark" blurAmount={60} className="rounded-2xl p-4 mb-5 overflow-hidden">
        ...
        <Text className="text-xl font-bold text-white mt-5 mb-4">Blur Types</Text>
        ...
        <View className="flex-row flex-wrap justify-between mb-5">
          {BLUR_TYPES.map((blur) => (
            ...
            >
              <Text className="text-base font-bold text-white mb-1">{blur.name}</Text>
              <Text className="text-xs text-white/80">blurType="{blur.type}"</Text>
            </BlurView>
          ))}
        </View>

        <Text className="text-xl font-bold text-white mt-5 mb-4">Blur Intensity</Text>
